...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Total Leads', value: totalLeads, icon: Users, color: '#6C3BF5', bg: 'rgba(108,59,245,0.12)', sub: `${hot} hot leads`, subColor: '#68D391' },
    ...
    { label: 'Conversion Rate', value: totalLeads > 0 ? Math.round((hot/totalLeads)*100)+'%' : '0%', icon: TrendingUp, color: '#A78BFA', bg: 'rgba(167,139,250,0.12)', sub: 'Hot/Total', subColor: '#718096' },
    { label: 'Revenue', value: 'Rp 0', icon: DollarSign, color: '#68D391', bg: 'rgba(104,211,145,0.12)', sub: 'Bulan ini', subColor: '#718096' },
  ]

  if (loading) return (
    <DashboardLayout>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#4A5568' }}>Memuat data...</div>
    </DashboardLayout>
  )
          ...
          <div style={{ background: '#16161F', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 16, marginTop: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 24, fontWeight: 700, color: '#F1F5F9' }}>{totalLeads}</span>
              </div>
            </div>
          </div>
        </div>
